...
export function useTournaments({ page = 1, perPage = 10, tier, tiers } = {}) {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      setError(null);
      try {
        const { items, pagination } = await PandaScore.csgoTournaments({ page, perPage, tier, tiers });
        if (!cancelled) {
  ...
  }, [page, perPage, tier, JSON.stringify(tiers || [])]);
